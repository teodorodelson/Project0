...
GetUsers.get('/getallAccounts', async (req, res, next) => {
    ...
    try {
        if (req.session && req.session.user !== undefined) {
            ...
            const result = await client.query('SELECT id, username, email, lname, fname, join_date, contact FROM user_accounts;')
...
GetUsers.post('/register', async (req, res, next) => {
    ...
    try {
        if (req.session && req.session.user !== undefined) {
            ...
            const result = client.query(`INSERT INTO user_accounts(
